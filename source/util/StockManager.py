...
class StockCode():
    ...
    def is_trade(self, debug):
        ...
        print_log = 'index[%s] 코드[%s] 시간[%s] 현재가[%s] 등락율[%s] 누적거래량[%s] ' \
                    '거래량차[%s] 거래대금[%s] 현재가차이[%s] 누적거래량차이[%s] 시간차이[%s]' % (
            self.index, self.df['코드'][self.index], self.df['시간'][self.index], self.df['현재가'][self.index], self.df['등락율'][self.index], self.df['누적거래량'][self.index],
            self.df['거래량차'][self.index], self.df['거래대금'][self.index], 현재가차이, 누적거래량차이, 시간차이)
